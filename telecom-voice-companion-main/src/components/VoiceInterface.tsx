...
const VoiceInterface = ({ sessionId }: VoiceInterfaceProps) => {
  ...
  const connectWebSocket = useCallback(() => {
    const ws = new WebSocket(WS_ENDPOINTS.VOICE(sessionId));
    wsRef.current = ws;
  ...
  const handleWebSocketEvent = (data: any) => {
    switch (data.type) {
      ...
      case "transcript":
        ...
        break;
      case "token":
        // Token streaming handled via chat history
        break;
      case "audio_start":
      ...
      case "response_complete":
        ...
        break;
      case "stop_audio":
        // Barge-in: clear audio queue and stop playback
  ...
  const playNextInQueue = useCallback(async () => {
    ...
    try {
      // Reuse or create audio context for playback
      ...
      source.onended = () => {
        ...
        if (audioQueueRef.current.length > 0) {
          ...
          // No more audio, set to listening
          setOrbState("listening");
        }
      };
  ...
  // Clear audio queue on barge-in/stop
  ...
  const stopVoice = () => {
    // Clear audio queue to stop any playing audio
    clearAudioQueue();
  ...
  return (
    <div className="fixed inset-0 bg-black flex">
      ...
      <div
        className={`relative transition-all duration-300 ease-out ${isPanelOpen ? 'w-80 lg:w-96' : 'w-0'
          }`}
      >
        {/* Toggle Button */}
        <button
